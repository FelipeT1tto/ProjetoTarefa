...
def update_task(task_id, title, description, date, importance):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE tasks SET title=?, description=?, date=?, importance=? WHERE id=?
...
def update_note(user_id, content):
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("REPLACE INTO notes (user_id, content) VALUES (?, ?)", (user_id, content))
    conn.commit()
    conn.close()
